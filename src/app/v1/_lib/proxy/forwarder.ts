...
export class ProxyForwarder {
  ...
  static async send(
    ...
    try {
      return await ProxyForwarder.sendInternal(session, options);
    } catch (error) {
  ...
  private static async sendInternal(
    ...
    const useStreamingHedge =
      options.allowProviderSwitch !== false && ProxyForwarder.shouldUseStreamingHedge(session);
