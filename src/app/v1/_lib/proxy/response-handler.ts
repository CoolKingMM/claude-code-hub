...
async function filterProviderOutputSafetySnapshotText(
  response: Response,
  text: string
): Promise<string> {
  const config = await getProviderOutputSafetyRuntimeConfig();
  ...
  return filterProviderOutputSafetyText(text, config.rules);
}
...
function finalizeDeferredStreamingFinalizationIfNeeded(
  ...
  const shouldClearSessionBindingOnFailure =
    (!streamCompletedForFinalization && !clientAbortCompleteSuccess) ||
    detected.isError ||
  ...
  if (!streamCompletedForFinalization && !clientAbortCompleteSuccess) {
...
export class ProxyResponseHandler {
  ...
  private static async handleNonStream(
    ...
    // 在后台统计任务启动前固定客户端最终响应，避免快速非流式响应先完成
    // after 快照、后套安全过滤器所造成的竞态。
    ...
    const runProcessingTask = async () => {
      ...
      try {
        ...
        if (session.sessionId && session.shouldPersistSessionDebugArtifacts()) {
          ...
          const afterBody = await filterProviderOutputSafetySnapshotText(
            finalResponse,
            clientVisibleResponseText
          );
          persistNonStreamAfterSnapshot(finalResponse, afterBody);
        }
  ...
  private static async handleStream(session: ProxySession, response: Response): Promise<Response> {
          ...
          try {
            if (session.request.model) {
              const resolvedPricing = await awaitFinalization(
                session.getResolvedPricingByBillingSource(effectiveProvider)
              );
              if (resolvedPricing) {
                ensurePricingResolutionSpecialSetting(session, resolvedPricing);
                const longContextPricing =
    ...
    let visibleClientStream = shouldHideCodexCyberNotice(session.provider?.providerType)
      ? activeResponsePump.stream.pipeThrough(createCodexCyberNoticeFilter())
